...
export class ServicoConfigController {
  async registraCatalagoServicoControler(req: Request, res: Response) {
    if (!req.user) {
      return res.status(401).json({ mensagem: "Token inválido" });
    }

    try {
      const salvaReqData = await servicoConfigService.registraServiceCatalago(
        req.body,
        req.user.empresaId
      );
  ...
  async buscarListaDeControler(req: Request, res: Response) {
    if (!req.user) {
      return res.status(401).json({ mensagem: "Token inválido" });
    }

    try {
      const lista = await servicoConfigService.buscarListaDeServices(
        req.user.empresaId
      );
  ...
  async deleteServicoControler(req: Request, res: Response) {
    if (!req.user) {
      return res.status(401).json({ mensagem: "Token inválido" });
    }
