...
export default function AbaEntradas() {
  ...
  const fetchColaboradores = async (token?: string) => {
    ...
    try {
      const response = await fetch(URLCOLABORADORES, {
        headers: { Authorization: `Bearer ${validToken}` },
      });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === "categoriaId") {
      ...
      setFormData((prev) => ({
        ...
        nomeCategoria: categoriaSelecionada ? categoriaSelecionada.nomeCategoria : "",
      }));
      return;
    }
    ...
    if (name === "colaboradorId") {
      setFormData((prev) => ({
        ...prev,
        colaboradorId: value ? Number(value) : undefined,
      }));
      return;
    }
  ...
  return (
    ...
    <div className={styles.containerFinanceiro}>
      ...
      <div className={styles.containerFinanceiroHeader}>
        <h3 className={styles.titleFinanceiro}>Entradas / Saídas Financeiras</h3>
          ...
          {entradasFiltradas.length > 0 ? (
            entradasFiltradas.map((entrada) => {
              const colaboradorNome =
                colaboradores.find((c) => c.id === entrada.colaboradorId)?.nomeCompleto || "—";
              return (
                <tr key={entrada.id}>
