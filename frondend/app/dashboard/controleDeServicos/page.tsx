...
export default function ControleDeServicos() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === "valorDoServico" || name === "colaboradorId" ? Number(value) : value,
    }));
  };
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.containerServicoHeader}>
          ...
          {servicosFiltrados.length > 0 ? (
            servicosFiltrados.map((servico) => (
              <tr key={servico.id}>
                ...
                <td>{catalagoDeSERVICOS.find(c => c.id === servico.servicoConfigId)?.tipo || ""}</td>
                <td>
