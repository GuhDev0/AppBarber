...
export default function AnalycisBarber() {
  ...
  const faturamentoMesAtual = faturamentoMensalLiquido.find(
    (item: any) => item.data === mesAtualConvertido
  );

  const filtraAtendimentoColaboradorRankMesAtual = rankingMensal.filter(
    (item: any) => item.data === mesAtualConvertido
  );

  const filtrarTotalDeServicosBarbeariaMesAtual = totalServicosMensal.filter(
    (item: any) => item.data === mesAtualConvertido
  );

  const filtraServicoMaisRealizadoMesAtual = servicosMaisRealizados.filter(
    (item: any) => item.data === mesAtualConvertido
  );
    ...
    <div className={styles.container}>
      ...
      <header className={styles.dashboardHeader}>
        ...
        <CardAnalytics
          titulo="Ticket Médio"
          valor={
            analiseBarbearia?.analise?.ticket_medio
              ? analiseBarbearia.analise.ticket_medio.toLocaleString("pt-BR", {
                  style: "currency",
                  currency: "BRL",
                })
              : "R$ 0,00"
        ...
        <CardAnalytics
          ...
          valor={
            faturamentoMesAtual?.valor_liquido
              ? faturamentoMesAtual.valor_liquido.toLocaleString("pt-BR", {
                  style: "currency",
                  currency: "BRL",
                })
              : "R$ 0,00"
          ...
          <div className={styles.rankingTableWrapper}>
            ...
            <table className={styles.rankingTable}>
              ...
              <tbody>
                ...
                {filtraAtendimentoColaboradorRankMesAtual.map(
                  (item: any, index: number) => (
                    <tr key={index}>
                      ...
                      <td className={styles.colaboradorName}>
                        {item.nomeCompleto}
                      </td>

                      <td className={styles.totalAtendimentos}>
                        {item.totalDeAtendimento}
                      </td>
                    </tr>
                  )
                )}
